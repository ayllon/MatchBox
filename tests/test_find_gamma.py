...
def test_is_quasi_clique(hyperclique48):
    ...
    assert is_quasi_clique(hyperclique48, {1, 2, 5}, lambd=.6, gamma=.5)
    assert is_quasi_clique(hyperclique48, {3, 4, 5}, lambd=.6, gamma=.5)
    assert is_quasi_clique(hyperclique48, {1, 2, 3, 4}, lambd=.6, gamma=.5)
    # We have 6 edges, comb(5,3) = 10, so gamma 0.6 must pass
    assert not is_quasi_clique(hyperclique48, {1, 2, 3, 4, 5}, lambd=0., gamma=0.8)
    assert is_quasi_clique(hyperclique48, {1, 2, 3, 4, 5}, lambd=0., gamma=0.3)
    # H0: every edge is equally likely to be missing. A lower lambd means
    #     the probability of rejecting H0 is lower
    assert not is_quasi_clique(hyperclique48, {1, 2, 3, 4, 5}, lambd=0.1, gamma=0.)
    assert is_quasi_clique(hyperclique48, {1, 2, 3, 4, 5}, lambd=0.01, gamma=0.)
